...
def download_image(session,img_url ,hashed_title):
   file_name=os.path.join("/HappyGames/news_imgs",hashed_title+".jpg")
   try:
...
def get_recent_news_ids(curs):
    try:
        curs.execute("SELECT news_id FROM news_pages ORDER BY created_date DESC LIMIT 15")
...
def connect_db():
    ...
    curs = None
    try:
        conn = pymysql.connect(host='127.0.0.1', user='root', password='1234', db='wworld', charset='utf8')
